...
mod tests {
    ...
    fn check_text_fragments_from_chrome() {
        ...
        let url = Url::parse("http://localhost:8080/#:~:text=Proin-,vulputate,-mi%20id%20sem").unwrap();
        assert!(check_text_fragments(&url, INDEX_HTML, FileType::Html));

        let url = Url::parse("http://localhost:8080/#:~:text=Proin-,vulputate,-mi%20id%20sema").unwrap();
        assert!(!check_text_fragments(&url, INDEX_HTML, FileType::Html));

        let url = Url::parse("http://localhost:8080/#:~:text=massa.,Proin").unwrap();
        assert!(check_text_fragments(&url, INDEX_HTML, FileType::Html));

        let url = Url::parse("http://localhost:8080/#:~:text=sit%20amet%20dignissim-,massa,-.").unwrap();
        assert!(check_text_fragments(&url, INDEX_HTML, FileType::Html));

        let url = Url::parse("http://localhost:8080/#:~:text=sit%20amet%20dignissim-,massa,-.").unwrap();
        assert!(check_text_fragments(&url, INDEX_HTML, FileType::Html));

        let url = Url::parse("http://localhost:8080/#:~:text=sit%20amet%20dignissim-,massam,-.").unwrap();
        assert!(!check_text_fragments(&url, INDEX_HTML, FileType::Html));
    }
}
